...
class C_GoodReceipt extends CI_Controller
{
    ...
    {
        $data = array(
            'title_form' => 'GoodReceipt',
            'url_back'   => site_url('C_GoodReceipt')
        );
        $data['datasat'] = $this->M_satuan->getdata();
        $data['datasup'] = $this->M_suplier->getdata();

        $this->load->view('template/header');
        $this->load->view('GoodReceipt/index', $data);
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $listdata) {
            if ($listdata->status == 2) {
                ...
                $row[] =  '<span style="color:white" class="badge bg-success"></i>Accepted</span>' . '<p>

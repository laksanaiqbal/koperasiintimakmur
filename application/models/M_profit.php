...
class M_profit extends CI_Model {
    ...
    var $table = 'hjual';
    var $column_order = array(null,null, 'a.tanggal','SubTotal','THpp','ProfitRp','ProfitPersen'); //set column field database for datatable orderable
    var $column_search = array('a.tanggal','SubTotal'); //set column field database for datatable searchable 
    var $order = array('a.tanggal' => 'asc'); // default order 
    ...
    private function _get_datatables_query()
    {
        // $month = date('m')-3;
        // $years = date('Y');
        $this->db->select("a.tanggal,round(sum(a.subtotal),2) SubTotal,round(sum(a.thpp),2) THpp,round(sum(a.subtotal)-sum(a.thpp),2) ProfitRp,ROUND(((sum(a.subtotal)-sum(a.thpp))/sum(a.thpp))*100,2) ProfitPersen");
    ...
    function get_datatables($params)
    {
        ...
        if (isset($params['txt_tgl_start']) && isset($params['txt_tgl_end'])) {

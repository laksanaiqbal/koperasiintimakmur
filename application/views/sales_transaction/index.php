        ...
        <div class="card">
            <div class="card-body">
                <form class="theme-form">
                    ...
                    <div class="mb-3 row">
                        <label class="col-sm-3 col-form-label" for="txt_nmkary">Customer Name</label>
                        <div class="col-sm-9">
                            <?php
                            $this->db->order_by('a.namacus', 'ASC');
                            $customers = $this->db->get('customer a'); ?>
                            <select id="txt_nmkary" name="txt_nmkary"
                                class="js-example-placeholder-multiple col-sm-12 select2">
                                <option value="">Silahkan Pilih</option>
                                <?php
                                foreach ($customers->result() as $rows) {
                                    echo "<option value='$rows->kodecus'>$rows->namacus</option>";
                                }
                                ?>
                            </select>
                        </div>
                    ...
                    <div class="mb-3 row">
                        <label class="col-sm-3 col-form-label" for="txt_status">Transactions Status</label>
                        <div class="col-sm-9">
                            <?php
                                $this->db->select("Distinct(b.deleted),case when b.deleted=2 then 'Cancel' when b.deleted=1 then 'Input' when b.deleted=0 then 'Oke' end as STATUS");
                                $this->db->from('djual b');
                                $this->db->order_by('b.deleted', 'ASC');
                                $statusbeli = $this->db->get(); ?>
                            <select id="txt_status" name="txt_status"
                                class="js-example-placeholder-multiple col-sm-12 select2">
                                <!-- <option value="">Silahkan Pilih</option> -->
                                <?php
                                foreach ($statusbeli->result() as $rowstatus) {
                                    echo "<option value='$rowstatus->deleted'>$rowstatus->STATUS</option>";
                                }
                                ?>
                            </select>
                        </div>
...
$(document).ready(function(e) {
    table = $('#datatable_list').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('C_sales_transaction/ajax_list') ?>",
            "type": "POST",
            "data": function(data) {
                $('#loader').hide();
                ...
                data.txt_status = $('#txt_status').select2().val();
            }
        },
...
$('#btn_reset').click(function() { //button reset event click
    ...
    $('[name="txt_status"]').select2().val('0').trigger('change');

            ...
            <div class="card-body">
                <form class="theme-form">
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-9">
                            <?php
                            $this->db->select("idcabang,namacabang");
                            $this->db->from('cabang b');
                            $this->db->order_by('idcabang', 'ASC');
                            $cabangs = $this->db->get(); ?>
                            <select id="txt_nmkary" name="txt_nmkary">
                                <option value="">Silahkan Pilih</option>
                                <?php
                                foreach ($cabangs->result() as $rowcabangs) {
                                    echo "<option value='$rowcabangs->idcabang'>$rowcabangs->namacabang</option>";
                                }
                                ?>
                            </select>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(e) {
        table = $('#datatable_list').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('C_daftardepartemen/ajax_list') ?>",
                "type": "POST",
                "data": function(data) {
                    $('#loader').hide();
